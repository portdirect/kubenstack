...
NEUTRON_SERVICE_HOST=${NEUTRON_SERVICE_HOST}
NOVA_METADATA_SERVICE_HOST=${NOVA_METADATA_SERVICE_HOST}
...
export cfg_ml2=/etc/neutron/plugins/ml2/ml2_conf.ini
export cfg_l3=/etc/neutron/l3_agent.ini
export cfg_lbaas=/etc/neutron/neutron_lbaas.conf
export cfg_lbaas_haproxy=/etc/neutron/services/loadbalancer/haproxy/lbaas_agent.ini
export cfg_metadata=/etc/neutron/metadata_agent.ini
export cfg_metadata_ovn=/etc/neutron/ovn_metadata_agent.ini
...
crudini --set $cfg DEFAULT global_physnet_mtu "1400"
...
crudini --set $cfg DEFAULT service_plugins "${L3_ROUTER_PLUGIN},neutron.services.qos.qos_plugin.QoSPlugin,neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2"
...
crudini --set $cfg agent extensions "qos"
...
crudini --set $cfg_ml2 agent extensions "qos"
crudini --set $cfg_ml2 ml2 tenant_network_types "geneve"
crudini --set $cfg_ml2 ml2 extension_drivers "qos,port_security"
...
crudini --set $cfg_ml2 ovn ovn_l3_mode "${OVN_L3_MODE}"
crudini --set $cfg_ml2 ovn ovn_l3_scheduler "leastloaded"
crudini --set $cfg_ml2 ovn ovn_native_dhcp "True"
...
crudini --set $cfg_metadata DEFAULT publish_errors "True"
crudini --set $cfg_metadata DEFAULT nova_metadata_port "8775"
crudini --set $cfg_metadata DEFAULT nova_metadata_ip "${NOVA_METADATA_SERVICE_HOST}"
crudini --set $cfg_metadata DEFAULT debug "True"
crudini --set $cfg_metadata AGENT root_helper_daemon "sudo /usr/bin/neutron-rootwrap-daemon /etc/neutron/rootwrap.conf"
crudini --set $cfg_metadata AGENT root_helper "sudo /usr/bin/neutron-rootwrap /etc/neutron/rootwrap.conf"
crudini --set $cfg_metadata DEFAULT metadata_proxy_shared_secret "supersecret"
crudini --set $cfg_metadata_ovn DEFAULT publish_errors "True"
crudini --set $cfg_metadata_ovn DEFAULT nova_metadata_port "8775"
crudini --set $cfg_metadata_ovn DEFAULT nova_metadata_ip "${NOVA_METADATA_SERVICE_HOST}"
crudini --set $cfg_metadata_ovn DEFAULT debug "True"
crudini --set $cfg_metadata_ovn DEFAULT metadata_proxy_shared_secret "supersecret"
